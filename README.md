...
| `sidesWrap`                     | <p>Characters placed around the sides part.</p><p>**Type:** `array` of two strings<br>**Example:** `["", ""]` for none, `["— ", ""]`<br>**Default value:** `["(", ")"]` → `(Sides: Corn and Apple)`</p>|
| `alternativeWrap`               | <p>Characters placed around each alternative meal.</p><p>**Type:** `array` of two strings<br>**Example:** `["", ""]` for none<br>**Default value:** `["[", "]"]` → `[Or PBJ Uncrustable Sandwich]`</p>|
| `partSeparator`                 | <p>In the `sentence` layout, the separator placed between the main meal, the sides, and each alternative.</p><p>**Type:** `string`<br>**Example:** `" | "` or `". "`<br>**Default value:** `" "`</p>|
...
**Tip for busy screens:** `layout: "sentence"` and `hideEverydaySides: true` (removes sides that appear every single day, like fruit and veggies) keep each day to a couple of lines; `showAlternatives: false` drops the "Or …" part and `showSides: false` the "Sides: …" part.
