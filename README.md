...
| `layout`                        | <p>How each meal is displayed.</p><p>**Type:** `string`<br>**Default value:** `"lines"`<br>**Possible values:** `"lines"` shows the main meal (with its sides) on one line and each alternative meal on an "or ..." line below it. `"sentence"` shows the same as one flowing paragraph ("Main meal with sides. Or alternative."), which takes the least vertical space.</p>|
| `showAlternatives`              | <p>Show alternative meals (Choice 2, Grab & Go, Box Lunch).</p><p>**Type:** `boolean`<br>**Default value:** `true`</p>|
| `showSides`                     | <p>Fold the day's sides (fruit, vegetables, dessert) into the main meal line. Set to `false` to show entrees only.</p><p>**Type:** `boolean`<br>**Default value:** `true`</p>|
...
```
Monday
  Mandarin Orange Chicken over Fluffy Brown Rice with Steamed Broccoli and Fortune Cookie   ← main meal ("2-4 Elementary") + its sides ("Sides for All Entrees")
  or Yogurt Parfait with Granola Packet                                                    ← alternative meal ("2-4 Choice 2", "Grab & Go", "Box Lunch")
```

or, with `layout: "sentence"`, as one paragraph: *Mandarin Orange Chicken over Fluffy Brown Rice with Steamed Broccoli and Fortune Cookie. Or Yogurt Parfait with Granola Packet.*
...
**Tip for busy screens:** `layout: "sentence"` and `hideEverydaySides: true` (removes sides that appear every single day, like fruit and veggies) keep each day to a couple of lines; `showAlternatives: false` drops the "Or …" part entirely.
